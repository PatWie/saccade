...
void Utils::ImageData::write(std::string filename) const {
	...
	for (int c = 0; c < _channels; ++c) {
		for (int h = 0; h < _height; ++h) {
			for (int w = 0; w < _width; ++w) {
				pixels[h * _width * _channels + w * _channels + c] = (BYTE) 255. * _raw_buf[(_channels - c - 1) * (_height * _width) + h * _width + w];
			}
		}
	}
	...
	const int scanwidth = _width * 3;

	FIBitmapPtr Image = FreeImage_ConvertFromRawBits(pixels, _width, _height,
	                  scanwidth, bpp,
